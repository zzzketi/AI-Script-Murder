...
# 引入Data 处理
from scripts_data import SCRIPT_LIST_DATA, get_script_detail
...
def list_scripts(keyword: Optional[str] = None):
    results = []
    for item in SCRIPT_LIST_DATA:
        # 简单的内存搜索
        if keyword:
            kw = keyword.lower()
            if kw not in item["title"].lower() and kw not in item["desc"].lower():
                continue
        results.append(ScriptBrief(**item))

    return ScriptListResponse(scripts=results)


@app.get("/scripts/{script_id}", response_model=ScriptDetail)
def get_script_detail_api(script_id: str):
    # 动态从文件读取
    detail = get_script_detail(script_id)
    if not detail:
        raise HTTPException(status_code=404, detail="Script not found")
    return detail


@app.get("/scripts/{script_id}/characters/{character_id}", response_model=Character)
def get_character_detail(script_id: str, character_id: str):
    """
    获取指定剧本下的特定角色详情
    """
    # 1. 先获取整个剧本信息
    ...
    # 2. 遍历剧本中的角色列表，查找匹配的 ID
    target_char = None
    for char in script_detail.characters:
...
def create_session(req: CreateSessionRequest):
    detail = get_script_detail(req.script_id)
    if not detail:
        raise HTTPException(status_code=404, detail="Script not found")

    # 简单校验角色 ID 是否存在于剧本中
    valid_ids = [c.id for c in detail.characters]
    if req.user_role_id not in valid_ids:
...
def list_all_sessions():
    ...
    for s in sessions:
        ...
        script_detail = get_script_detail(s.script_id)
        script_title = script_detail.title if script_detail else s.script_id
        brs.append(
            SessionBrief(
